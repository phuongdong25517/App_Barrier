  ...
  return (
    <View style={styles.lightWrapper}>
      <View style={[styles.light, {
        backgroundColor: active ? Colors.green : '#1a2a1a',
        borderColor: active ? Colors.green : '#333',
        shadowColor: active ? Colors.green : 'transparent',
        shadowOpacity: active ? 0.9 : 0, shadowRadius: active ? 6 : 0, elevation: active ? 4 : 0,
      }]} />
...
export function DataCard({ title, color, children }) {
  ...
  return (
    <View style={[styles.dataCard, { backgroundColor: Colors.card, borderColor: Colors.border, borderTopColor: c }]}>
...
export function ActionBtn({ label, iconSource, color, onPress, disabled, size = 'md' }) {
  const { Colors } = useTheme();
  const c = color || Colors.accent;
  const [pressed, setPressed] = useState(false);
  const sz = size === 'lg' ? 72 : 54;
  const iconSz = size === 'lg' ? 30 : 22;
  ...
  return (
    <TouchableOpacity disabled={disabled}
      ...
      {iconSource && (
        <Image source={iconSource} style={{ width: iconSz, height: iconSz }} resizeMode="contain" />
      )}
      <Text style={[styles.actionLabel, { color: disabled ? Colors.muted : c, fontSize: size === 'lg' ? 9 : 8 }]}>
        {label}
      </Text>
    </TouchableOpacity>
  );
...
export function Toggle({ value, onToggle }) {
  ...
  return (
    <TouchableOpacity onPress={() => onToggle(!value)}
      style={[styles.toggle, { backgroundColor: value ? Colors.accent : Colors.border }]} activeOpacity={0.8}>
...
export function Stepper({ value, min, max, onChangeValue, unit = '' }) {
  ...
  return (
    <View style={styles.stepper}>
      <TouchableOpacity onPress={() => value > min && onChangeValue(value - 1)}
      ...
      <TouchableOpacity onPress={() => value < max && onChangeValue(value + 1)}
        style={[styles.stepBtn, { backgroundColor: Colors.border }]}>
        <Text style={[styles.stepBtnText, { color: Colors.text }]}>+</Text>
      </TouchableOpacity>
    </View>
...
const styles = StyleSheet.create({
  ...
  dataUnit: { fontSize: 9 },
  actionBtn: { borderRadius: 14, borderWidth: 1.5, alignItems: 'center', justifyContent: 'center', gap: 4 },
  actionLabel: { fontWeight: '700', letterSpacing: 0.5 },
  ...
  stepValue: { minWidth: 48, textAlign: 'center', fontFamily: 'monospace', fontSize: 13 },
});
